...
function horiBarChart_PrepData(dataObj) {
  //DATA SORT
...
function HorizontalBarChart(
  ...
  svg
    ...
    .call((g) =>
      g
        .append("text") //this is the "frequency" on x axis
        .attr("id", "xAxisText")
...
function horiBarChart_ChartResize(
  svgDivId,
  {
    marginLeft,
    marginRight,
  ...
  d3.select(svgDivId)
    ...
    .call((text) =>
      text
        .filter((i) => xScale(X[i]) - xScale(0) < 25) // short bars
        .attr("dx", +4)
    ...
    .call((text) =>
      text
        .filter((i) => xScale(X[i]) - xScale(0) >= 25) // long bars
        .attr("dx", -4)
        .attr("fill", titleColor)
        .attr("text-anchor", "end")
    );
